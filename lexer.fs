...
module lexer =
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               if  lookahead = '.' then // check for real
                  let (lit2, look2) = collectIntLiteral (nextChar input) input ""
                  lexer look2 input (("real_literal:"+literal+"."+lit2)::tokens) // formatting real
               else // not real
